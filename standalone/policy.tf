...
locals {
    ec2_full_statement = jsondecode(data.aws_iam_policy_document.ec2_full.json)["Statement"]
    s3_full_statement = jsondecode(data.aws_iam_policy_document.s3_full.json)["Statement"]
}
